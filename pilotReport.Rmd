...
```{r}
articleID <- "1-2-2016" # insert the article ID code here e.g., "10-3-2015_PS"
reportType <- "pilot" # specify whether this is the 'pilot' report or 'final' report
pilotNames <- "Daphna Spivack" # insert the pilot's name here e.g., "Tom Hardwicke".  If there are multiple cpilots enter both names in a character string e.g., "Tom Hardwicke, Bob Dylan"
copilotNames <- "Zainab Hosseini" # insert the co-pilot's name here e.g., "Michael Frank". If there are multiple co-pilots enter both names in a character string e.g., "Tom Hardwicke, Bob Dylan"
pilotTTC <- 600 # insert the pilot's estimated time to complete (in minutes, fine to approximate) e.g., 120
copilotTTC <- NA # insert the co-pilot's estimated time to complete (in minutes, fine to approximate) e.g., 120
pilotStartDate <- "11/3/2018" # insert the pilot's start date in US format e.g., as.Date("01/25/18", format = "%m/%d/%y")
copilotStartDate <- NA # insert the co-pilot's start date in US format e.g., as.Date("01/25/18", format = "%m/%d/%y")
completionDate <-"10/4/2018" # copilot insert the date of final report completion (after any necessary rounds of author assistance) in US format e.g., as.Date("01/25/18", format = "%m/%d/%y")
```
...
#Creating a Data set for only OIssue *PROBLEM HERE*  
OSpecificIssue_kleimantidy = kleimantidy %>%
  select(c("Pnum", "Condition", "Response", "OSpecific_Issue"))

OSpecificIssue_kleimantidy <- subset(OSpecificIssue_kleimantidy, OSpecific_Issue != "")
...
SpecificIssuecomposite <- merge(Kcomposite_RSpecificIssue, Kcomposite_OSpecificIssue) %>%
  mutate(delta_SpecificIssue = RSpecificIssue_composite - OSpecificIssue_composite)
```
...
#t.test for Issue Means  
t.test(delta_SpecificIssue ~ Condition, 
       data = SpecificIssuecomposite) 
...
I was able to reproduce both the t-test between the two idealogy difference means and issue difference means. My results alligned with the authors' findings. The only error was a rounding error: the authors found a t value of 2.38 for the specific issues t.test when it should have been 2.37. 
